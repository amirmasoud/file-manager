...
class UploadController extends LfmController
{
    ...
    public function upload(Request $request)
    {
        ...
        $error_bag = [];
        $covert_to_m3u8 = ($request->has('convert_m3u8') ? true : false);
        foreach (is_array($files) ? $files : [$files] as $file) {
            $validation_message = $this->uploadValidator($file);
            $new_filename = $this->proceedSingleUpload($file, $covert_to_m3u8);
    ...
    private function proceedSingleUpload($file, $covert_to_m3u8)
    {
        $validation_message = $this->uploadValidator($file);
        if ($validation_message !== 'pass') {
